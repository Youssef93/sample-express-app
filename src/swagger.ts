...
export const appendEndpointToSwaggerDoc = (routeConfig: IRoute): void => {
  ...
  if (routeConfig.schema) {
    ...
    if (routeConfig.swagger.parameters) {
      swaggerDoc.paths[routeConfig.endpoint][routeConfig.method]['parameters'] = routeConfig.swagger.parameters.map(
        ({ name, pathOrQuery, required }) => ({
          name,
          required,
          in: pathOrQuery,
          schema: {
    ...
    if (routeConfig.swagger.security) {
      swaggerDoc.paths[routeConfig.endpoint][routeConfig.method]['security'] = routeConfig.swagger.security.map(
        security => {
...
export const writeSwaggerDocument = (): void => {
